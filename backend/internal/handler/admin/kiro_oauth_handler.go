...
// KiroOAuthHandler 处理 Kiro 账号的 OAuth 授权相关管理接口。
...
// GenerateAuthURL 生成 Kiro 社交登录授权链接。
...
// GenerateIDCAuthURL 生成 Kiro AWS IAM Identity Center 企业登录授权链接。
...
// ExchangeCode 用授权码交换 Kiro token。
...
// RefreshToken 手动刷新 Kiro token（用于创建账号前的连通性校验）。
...
// ImportToken 直接导入已有的 Kiro token JSON（跳过 OAuth 流程）。
