...
type Account struct {
	...
	// Kiro runtime usage state is derived from the local usage/cooldown cache and is never persisted.
...
type AdminUsageLog struct {
	...
	// KiroSessionFingerprint is an irreversible correlation identifier for Kiro usage auditing.
	KiroSessionFingerprint *string `json:"kiro_session_fingerprint,omitempty"`
	// PreviousKiroAccountID identifies the prior actual Kiro account in the same session.
	PreviousKiroAccountID *int64 `json:"previous_kiro_account_id,omitempty"`
