...
func TestGatewayModels_KiroGroupFallsBackToKiroModels(t *testing.T) {
	...
	h := newGatewayModelsHandlerForTest(
		&gatewayModelsAccountRepoStub{
			byGroup: map[int64][]service.Account{
				groupID: {
					{ID: 1, Platform: service.PlatformKiro, Type: service.AccountTypeOAuth},
				},
			},
		},
	)
	...
	c.Request = httptest.NewRequest(http.MethodGet, "/v1/models", nil)
	c.Set(string(middleware2.ContextKeyAPIKey), &service.APIKey{
		Group: &service.Group{ID: groupID, Platform: service.PlatformKiro},
	})
...
func TestGatewayModels_KiroCustomModelsListAllowsGPT56ForAPIKey(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	var got gatewayModelsResponseForTest
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &got))
	require.Equal(t, []string{"gpt-5.6-sol"}, modelIDsForTest(got.Data))
}
