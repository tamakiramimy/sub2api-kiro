...
func apiKeyAuthWithSubscription(apiKeyService *service.APIKeyService, subscriptionService *service.SubscriptionService, cfg *config.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Model catalogs and async image task polling only read metadata/data that
		// already belongs to the authenticated key. They must remain available
		// after the key's balance is exhausted so clients can configure providers
		// and inspect completed work.
		modelListRequest := c.Request.Method == http.MethodGet && (c.Request.URL.Path == "/v1/models" || c.Request.URL.Path == "/models")
		skipBilling := c.Request.URL.Path == "/v1/usage" || billingInfoRequest || modelListRequest || isAsyncImageTaskRead(c.Request.Method, c.Request.URL.Path)
