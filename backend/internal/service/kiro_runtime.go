...
func (s *GatewayService) forwardKiroMessages(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest, startTime time.Time) (*ForwardResult, error) {
	...
	// kiroGroup 用于 Kiro cache emulation 等分组级配置，不依赖 ParsedRequest.Group（该共享结构体不携带完整 Group 对象），而是根据 GroupID 现取。
	var kiroGroup *Group
	if parsed.GroupID != nil && s.groupRepo != nil {
		if g, err := s.groupRepo.GetByIDLite(ctx, *parsed.GroupID); err == nil {
			kiroGroup = g
		}
	}
	...
	if parsed.Stream {
		resp, _, err := s.openKiroAnthropicStreamResponse(ctx, account, parsed, body, mappedModel, originalModel, c.Request.Header, kiroGroup)
		if err != nil {
	...
	cacheUsage := s.buildKiroCacheEmulationUsage(account, kiroGroup, body, mappedModel, inputTokens)
