...
const kiroClaude5LegacyClientAliases = new Map([
  ['claude-opus-5-0', 'claude-opus-5'],
...
export function sanitizeKiroModelMapping(rawMapping?: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [legacyModel, clientModel] of kiroClaude5LegacyClientAliases) {
    if (legacyModel in mapping && !(clientModel in mapping)) {
      mapping[clientModel] = kiroClaude5CanonicalModelIDs.get(clientModel)!
    }
  }
  return mapping
}
