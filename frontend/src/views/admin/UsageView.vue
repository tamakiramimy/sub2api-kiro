...
const filters = ref<AdminUsageQueryParams>({ user_id: undefined, model: undefined, group_id: undefined, request_type: undefined, native_compaction_v2: null, billing_type: null, kiro_session_fingerprint: undefined, start_date: startDate.value, end_date: endDate.value })
...
const resetFilters = () => {
  ...
  filters.value = { start_date: startDate.value, end_date: endDate.value, request_type: undefined, native_compaction_v2: null, billing_type: null, billing_mode: undefined, kiro_session_fingerprint: undefined }
  granularity.value = getGranularityForRange(startDate.value, endDate.value)
  applyFilters()
}
...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      t('admin.usage.requestId'), t('admin.usage.upstreamRequestId'), t('usage.userAgent'), t('admin.usage.ipAddress'), t('admin.usage.kiroSession')
    ]
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        log.created_at, log.user?.email || '', log.api_key?.name || '', log.account ? `${log.account.name} #${log.account.id} (${log.account.platform})` : '', log.model,
        ...
        log.request_id || '', log.upstream_request_id || '', log.user_agent || '', log.ip_address || '', log.kiro_session_fingerprint || ''
      ])
      if (rows.length) {
...
const allColumns = computed(() => [
  ...
  { key: 'kiro_session', label: t('admin.usage.kiroSession'), sortable: false },
